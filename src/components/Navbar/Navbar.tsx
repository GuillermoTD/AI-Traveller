...
const Navbar = () => {
  ...
  return (
    <div className="Navbar">
      <div className="logo font-bold text-[1.5rem] cursor-pointer" onClick={()=>navigate("/")} >SmarTravel</div>
      {/* <Button className="btn-black" label="Sign Up" onClick={() => setVisible(true)} /> */}
      {currentUser?.photoURL ? (
        <div className="Navbar_UseProfileImg">
      ...
      {visible ? (
        <div className="bg-slate-900 w-full h-full z-100 absolute top-0 left-0 opacity-[40%] rounded-lg">
          <Dialog
            className="bg-slate-200 max-w-[30rem] max-h-[15rem] h-[10rem] rounded-lg p-[2rem]"
            visible={visible}
            onHide={() => {
              if (!visible) return;
              setVisible(false);
          ...
          >
            <div className=" h-full flex flex-col items-center justify-center gap-[0.7rem]">
              ...
              <button
                className="btn-black flex gap-[0.5rem]"
                onClick={handleLogin}
              >
                <img
                  className="w-[1.5rem] h-[1.5rem]"
                  src="src/assets/google-svgrepo-com.svg"
                  alt="google-icon"
                />
